<script lang="ts">
  ...
  let querySummary = $derived(result ? `${count(result.total_rows)} rows, page ${result.page} of ${count(result.total_pages)}, ${filters.length} filters, ${sorts.length} sorts, and ${dedupeColumns.length} dedupe keys.` : '');
  ...
  async function changePage(next: number) { if (next < 1 || next > totalPages || next === page) return; page = next; await loadData(); }
  ...
  async function changePageSize(event: Event) { pageSize = Number((event.currentTarget as HTMLSelectElement).value); page = 1; await loadData(); }
...
<div class:rail-collapsed={railCollapsed} class="app-shell">
  <header class="topbar" inert={!!inspectorMode}>
    ...
    <div class="brand"><span aria-hidden="true">D</span><strong>DuckScope</strong></div>
  ...
  <div class="shell">
    ...
    <main>
      {#if !selectedNodeId}
        <section class="welcome"><div class="welcome-icon">D</div><h1>Explore local data</h1><p>Open a local file or a read-only DuckDB database. DuckScope keeps the work on this machine and loads only the page you are viewing.</p><ol class="onboarding-steps"><li><b>1. Add a source</b><span>CSV, TSV, Parquet, JSON, JSONL/NDJSON, XLSX, DuckDB, or DB</span></li><li><b>2. Choose a dataset</b><span>Tables and views appear after the source opens</span></li><li><b>3. Inspect the data</b><span>Filter, profile, hide columns, or dedupe by selected keys</span></li></ol>{#if error}<div class="error" role="alert"><div><strong>Could not load DuckScope</strong><p>{error}</p></div><button onclick={loadNodes}>Retry</button></div>{/if}<label class="primary-button">Choose a file<input type="file" accept=".csv,.tsv,.parquet,.json,.ndjson,.jsonl,.xlsx,.duckdb,.db" onchange={upload} disabled={mutating} /></label><details class="onboarding-attach"><summary>Attach a local DuckDB database</summary><form onsubmit={(event) => { event.preventDefault(); attach(); }}><label for="onboarding-database-path">Database path<input id="onboarding-database-path" bind:value={attachPath} placeholder="/data/example.duckdb" disabled={mutating} /></label><button class="secondary-button" type="submit" disabled={mutating || !attachPath.trim()}>Attach read-only database</button></form></details></section>
      {:else}
        <section class="workspace">
          <header class="dataset-head" inert={!!inspectorMode}>
            <div><h1>{currentDataset?.name ?? selectedNode?.name}</h1>{#if result}<p class="dataset-meta"><span>{result.total_rows.toLocaleString()} rows</span><span>{compact(result.elapsed_ms)} ms</span></p>{/if}</div>
            <button class="icon-button" onclick={loadData} disabled={loadingData} aria-label="Refresh data" title="Refresh data">↻</button>
          </header>
          {#if datasets.length > 1}<nav class="dataset-tabs" aria-label="Datasets">{#each datasets as dataset (dataset.id)}<button class:active={dataset.id === selectedDataset} aria-current={dataset.id === selectedDataset ? 'page' : undefined} onclick={() => selectDataset(dataset.id)}>{dataset.name}</button>{/each}</nav>{/if}
          ...
          {:else if selectedDataset}
            <section class="querybar" aria-label="Query controls" inert={!!inspectorMode}>
              ...
              {#if filters.length || sorts.length || dedupeColumns.length}<button class="clear-query" onclick={clearQuery}>Clear query</button>{/if}
            </section>
